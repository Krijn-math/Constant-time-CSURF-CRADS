...
# random_key() implements an uniform random sample from S(m_1) x S(m_2) x ... x S(m_n)
            ...
            if isinfinity(ramifications[-1][0]) == False:
                ...
                for j in range(0, len(moves) - 1, 1):
                    ...
                    if global_L[pos] <= HYBRID_BOUND:
                        # This branchs corresponds with the use of the tradicional velu's formulae
                        # T_- (or T_)
                        ramifications[j][0] = xEVAL(ramifications[j][0], pos)
                        # T_+ or (T_+)
                        ramifications[j][1] = xEVAL(ramifications[j][1], pos)
                    else:
                        # This branchs corresponds with the use of the new velu's formulae
                        # T_- (or T_)

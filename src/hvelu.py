...
SCALED_REMAINDER_TREE = { 'unscaled': False, 'scaled': True }[setting.multieval]
...
try:
    if setting.verbose:
        ...
        sI_list = []
        sJ_list = []
        f = open('./ijk/' + setting.prime + '-' + setting.multieval)
...
if setting.units:
    if (sys.argv[0] != 'header.py'):
        print("// Measuring the cost of a degree-ell isogeny construction and evaluation for each small odd prime number ell in the factorization of p + 1")
    cISOG_and_cEVAL()                                       # Precomputing the isogeny constructions and evaluations for each l in L